...
public class GesuchtGefunden {
	...
	public static void main(String[] args) {
        ...
        System.out.println("Das größte Element ist: " + groestesElement);
        ...
        System.out.println("Das kleinste Element ist: " + kleinstesElement);

        double gesamt = 0;
        for (int i = 0; i < arr.length; i++) {
             gesamt += arr[i];
        }
        System.out.println("Der Gesamtwert der Zahlen ist: " + gesamt);

        double paardiff = 0;
        for (int i = 0; i < arr.length -1; i++) {
        double klPaardiff = arr[i + 1] - arr[i];
        if (klPaardiff < paardiff){
            paardiff = klPaardiff;
        }
        }
        System.out.println("Die kleinste Paardifferenz ist: " + paardiff);
	}
